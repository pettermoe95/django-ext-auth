...
class ExtAuthBackendTests(TestCase):
    ...
    def test_user_exists(self):
        ...
        aad_backend = AzureADBackend()
        aad_backend.create_user(username, email)
        user_exists = aad_backend.user_exists(username)
    ...
    def test_migrate_old_user(self):
        username = 'oldusername@email.com'
        new_username = 'new_username'

        aad_backend = AzureADBackend()
        aad_backend.create_user(username, username)

        # Checking that it exists before migration
        user_exists = aad_backend.user_exists(username)
        self.assertEqual(user_exists, True)
        ext_auth.migrate_old_user(new_username, username)

        # Checking that it does not exist after migration
        user_exists = aad_backend.user_exists(username)
        self.assertEqual(user_exists, False)

        # Checking that we find the user with new username after migration
        user_exists = aad_backend.user_exists(new_username)
        self.assertEqual(user_exists, True)
